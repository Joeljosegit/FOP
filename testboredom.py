...
def main():
    ...
    for timestep in range(10):
        ...
        for c in creatures:
                ...
                c.hunger_depletion()
                c.boredom_fill() #7.0.4 - fills the creature's boreodm each time-step
                c.hunger_level() 
                c.boredom_level() #7.0.5 - displays the creatures' current boredom level as a label
                c.play(creatures,friends)
                c.play_toy(creatures,toys) #3.2.1.2 - when the toy is near the dog and the dog is bored the dog plays with the toy, by carrying it
                ...
                c.plot_me(ax[0],size)
                c.plot_hunger_boredom(creatures,ax[2]) #7.0.6 - plots the boredom and hunger labels onto a scatter plot with different colours corresponding to a different label
